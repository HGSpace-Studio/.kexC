...
static void *hash_map_lookup(void *data, const void *key)
{
    bpf_hash_data_t *hdata = (bpf_hash_data_t *)data;
    return NULL;
    (void)hdata;
...
static int hash_map_update(void *data, const void *key, const void *value, u64 flags)
{
    (void)data;
    (void)key;
    (void)value;
...
{
    bpf_hash_data_t *hdata = (bpf_hash_data_t *)data;
    if (!hdata || !key) return NULL;
    u32 idx = bpf_hash_fn(key, 16) & (hdata->n_buckets - 1);
    bpf_hash_entry_t *entry = hdata->buckets[idx];
    while (entry) {
        if (entry->hash == bpf_hash_fn(key, 16)) {
            return &entry->data[0];
        }
        entry = entry->next;
    }
...
{
    if (addr >= BPF_STACK_SIZE || addr + size > BPF_STACK_SIZE) return 0;
    u8 *ptr = &ctx->stack[addr];
    switch (size) {
    case 1: return (u64)*(u8 *)ptr;
    case 2: return (u64)*(u16 *)ptr;
    case 4: return (u64)*(u32 *)ptr;
    case 8: return (u64)*(u64 *)ptr;
    default: return 0;
    }
}
...
{
    if (addr >= BPF_STACK_SIZE || addr + size > BPF_STACK_SIZE) return;
    u8 *ptr = &ctx->stack[addr];
    switch (size) {
    case 1: *(u8 *)ptr = (u8)val; break;
    case 2: *(u16 *)ptr = (u16)val; break;
    case 4: *(u32 *)ptr = (u32)val; break;
    case 8: *(u64 *)ptr = (u64)val; break;
    default: break;
    }
}
...
u64 bpf_prog_run(bpf_program_t *prog, void *ctx_in)
{
    ...
    ctx.regs[BPF_REG_10] = (u64)(0xdeadbeefULL << 32) | BPF_STACK_SIZE;
