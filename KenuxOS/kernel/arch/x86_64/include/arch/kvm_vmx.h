...
struct kvm_run {
    ...
    uint32_t ready_for_interrupt_injection;
    uint64_t cr8;
    uint64_t apic_base;
    uint64_t pcr_entries;
...
struct kvm_vm {
    ...
    uint64_t eptp;
    void* ept_root;
    uint64_t ept_root_phys;
    uint64_t vmxon_region;
    void* vmxon_virt;
    uint64_t secondary_exec_control;
    spinlock_t lock;
    int active;
};
...
struct kvm_arch {
    int vmx_enabled;
    uint64_t vmx_basic;
    uint64_t ept_capabilities;
    uint64_t pinbased_ctls;
    uint64_t procbased_ctls;
    uint64_t exit_ctls;
    uint64_t entry_ctls;
    uint64_t basic_cpu_based_exec_ctrl;
    uint64_t basic_pin_based_exec_ctrl;
    uint64_t basic_vm_exit_ctrl;
    uint64_t basic_vm_entry_ctrl;
};
...
{
    uint64_t val;
    __asm__ volatile ("rdmsr" : "=a"((uint32_t)val), "=d"((uint32_t)(val >> 32)) : "c"(msr));
    return val;
}

static inline uint64_t adjust_vmx_controls(uint64_t ctl, uint64_t msr)
{
    uint64_t vmx_msr_low, vmx_msr_high;
    __asm__ volatile ("rdmsr" : "=a"((uint32_t)vmx_msr_low), "=d"((uint32_t)vmx_msr_high) : "c"(msr));
    ctl &= vmx_msr_high;
    ctl |= vmx_msr_low;
    return ctl;
}
...
int kvm_init(void);
int kvm_vm_create(struct kvm_vm* vm);
void kvm_vm_destroy(struct kvm_vm* vm);
int kvm_vcpu_create(struct kvm_vm* vm, uint32_t vcpu_id, struct kvm_vcpu* vcpu);
void kvm_vcpu_destroy(struct kvm_vcpu* vcpu);
int kvm_vcpu_run(struct kvm_vcpu* vcpu);
int kvm_vcpu_setup(struct kvm_vcpu* vcpu);

void kvm_vmx_setup_vmcs(struct kvm_vcpu* vcpu);
void kvm_vmx_exit_handler(struct kvm_vcpu* vcpu);

uint64_t kvm_ept_alloc_page(struct kvm_vm* vm);
void kvm_ept_free_page(struct kvm_vm* vm, uint64_t page);
int kvm_ept_map_page(struct kvm_vm* vm, uint64_t gpa, uint64_t hpa, uint64_t flags);
int kvm_ept_unmap_page(struct kvm_vm* vm, uint64_t gpa);
int kvm_set_memory_region(struct kvm_vm* vm, uint32_t slot,
                          uint64_t guest_phys_addr, uint64_t memory_size);

extern struct kvm_arch kvm_arch_state;
