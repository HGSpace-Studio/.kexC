...
OBJCOPY = os.path.join(MINGW_BIN, "objcopy.exe")
AS = os.path.join(NASM_PATH, "nasm.exe")
QEMU = os.path.join(QEMU_PATH, "qemu-system-x86_64.exe")
...
def collect_kernel_sources() -> tuple[list[Path], list[Path]]:
    ...
    if ninja_file.exists():
        ...
        # 自动扫描（fallback）
        c_sources = collect("kernel/kernel/*.c", "kernel/arch/x86_64/*.c",
                            "kernel/api/*.c", "kernel/lib/libc/*.c")
        asm_sources = collect("kernel/arch/x86_64/*.S", "kernel/arch/x86_64/*.s")
...
def qemu_command(*, debug: bool = False) -> tuple[str, ...]:
    ...
    # VVFAT 模式直接挂载 esp 目录
    command += ["-drive", f"file=fat:rw:{ESP_DIR},format=raw"]
...
def build_graph(paths: BuildPaths) -> BuildGraph:
    ...
    # (bootloader 跳转到 0x200000，即 .text 段起始处)
    start64_obj = None
    ...
    # ===== 2. 链接内核 (直接输出 flat binary) =====
    link_command = [
        CC, "-m64", "-T", relative(LINKER_SCRIPT),
        "-nostdlib", "-nodefaultlibs", "-nostartfiles",
        "-Wl,-e,_start", "-no-pie",
        "-Wl,--oformat,binary",
        "-o", relative(KERNEL_BIN),
    ] + [relative(obj) for obj in kernel_objects]

    add_link(graph, "kernel-image", KERNEL_BIN, kernel_objects, link_command,
